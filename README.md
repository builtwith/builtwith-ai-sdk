...
| `mcp_registry({ search?, category?, offset? })` | — | Search/browse the BuiltWith MCP registry (v1) of discovered remote MCP servers (no API credits) |
| `mcp_registry_v2({ search?, category?, offset? })` | — | Search/browse the BuiltWith MCP registry (v2) of discovered remote MCP servers, with per-endpoint auth flags and first/last detected dates (no API credits) |
...
| `whoami()` | — | Account limits, credit costs, endpoint inventory (no API credits) |
| `usage()` | — | Current credit balance (no API credits) |
| `mcp_registry(search?, category?, offset?)` | `string?`, `string?`, `int?` | Search/browse the BuiltWith MCP registry (v1) of discovered remote MCP servers (no API credits) |
| `mcp_registry_v2(search?, category?, offset?)` | `string?`, `string?`, `int?` | Search/browse the BuiltWith MCP registry (v2) of discovered remote MCP servers, with per-endpoint auth flags and first/last detected dates (no API credits) |
